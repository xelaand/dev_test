...
Процедура ОбработкаКоманды(ПараметрКоманды, ПараметрыВыполненияКоманды)
	...
	Если ПараметрКоманды.Количество() = 1 Тогда
		...
		ТекстВопроса = НСтр("ru = 'Персональные данные субъекта «%1» будут скрыты.
			|
			|Продолжить?'");
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстВопроса, ПараметрКоманды[0]);
...
Процедура СкрытьПерсональныеДанныеСубъектов(Субъекты)
	...
	ЗащитаПерсональныхДанных.СкрытьПерсональныеДанныеСубъектов(ОтбираемыеСубъекты, Истина);
