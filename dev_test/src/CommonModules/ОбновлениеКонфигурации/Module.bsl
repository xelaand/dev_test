			...
			Иначе
				Для Каждого ИнформацияОПрименимости Из СвойстваИсправления.AppliedFor Цикл
					...
					Если ВерсияБиблиотекиКонфигурации <> Неопределено Тогда
						СписокВерсийИсправления = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ИнформацияОПрименимости.Versions, ",", Истина);
						ПоследняяВерсия = СписокВерсийИсправления[0];
						Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ПоследняяВерсия, ВерсияБиблиотекиКонфигурации) >= 0 Тогда
							УдалитьИсправление = Ложь;
						КонецЕсли;
					КонецЕсли;
				КонецЦикла;
