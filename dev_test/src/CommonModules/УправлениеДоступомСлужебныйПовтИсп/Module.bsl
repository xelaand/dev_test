...
Функция КэшПараметровОграничения(КлючВерсии) Экспорт
	...
	Если КоличествоРеквизитовТабличнойЧасти = 0 Тогда
		ВызватьИсключение
			НСтр("ru = 'В справочнике КлючиДоступа в табличных частях ТабличнаяЧасть*
			           |может только одинаковое количество реквизитов Значение* и не более 15.'");
	КонецЕсли;
...
Функция КоличествоОпорныхПолейРегистра(ИмяРегистра = "") Экспорт
	...
	Для Каждого Измерение Из МетаданныеРегистраКлючей.Измерения Цикл
		Если Измерение.Имя = "Регистр" И ИмяРегистра = ""
		 Или Измерение.Имя = "ДляВнешнихПользователей"  Тогда
			Продолжить;
		КонецЕсли;
...
Функция ОписаниеПредопределенногоИдентификатораОбъектаМетаданных(ПолноеИмяОбъектаМетаданных) Экспорт
	...
	Имена = Метаданные.Справочники.ИдентификаторыОбъектовМетаданных.ПолучитьИменаПредопределенных();
	...
	Имена = Метаданные.Справочники.ИдентификаторыОбъектовРасширений.ПолучитьИменаПредопределенных();
	Если Имена.Найти(Имя) <> Неопределено Тогда
