...
Процедура ОткрытьФайлЗагрузки(КомпонентыОбмена, ИмяФайлаОбмена) Экспорт
	...
	Пока Не ПрерватьЦикл Цикл
		...
		Если КомпонентыОбмена.ТолькоНастройкиXDTO Тогда
			// Если получаем только настройки XDTO, в формате обмена будет только базовая схема, без версии.
			ЗаполнитьСтруктуруНастроекXDTOКорреспондента(КомпонентыОбмена.НастройкиXDTOКорреспондента, Header, Ложь);
		...
		Если ЭтоОбменЧерезПланОбмена Тогда
			...
			ЗаполнитьСтруктуруНастроекXDTOКорреспондента(КомпонентыОбмена.НастройкиXDTOКорреспондента, Header);
...
//
Функция ПоддерживаемыеОбъектыФормата(ИмяПланаОбмена, Режим = "ОтправкаПолучение") Экспорт
	...
	Если ЕстьАлгоритм Тогда
		ПланыОбмена[ИмяПланаОбмена].ПриОпределенииПоддерживаемыхОбъектовФормата(ТаблицаОбъекты, Режим);
	КонецЕсли;
...
// При построении таблицы учитывается текущая используемая в обмене версия формата.
//
// Параметры:
...
Функция ПоддерживаемыеОбъектыФорматаКорреспондента(УзелОбмена, Режим = "ОтправкаПолучение") Экспорт
	...
	ТаблицаОбъекты = Новый ТаблицаЗначений;
	ИнициализироватьТаблицуПоддерживаемыхКорреспондентомОбъектовФормата(ТаблицаОбъекты, Режим);
	
	ВерсияФормата = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(УзелОбмена, "ВерсияФорматаОбмена");
		...
		Если Выборка.Следующий() Тогда
			...
			Если Не НастройкиКорреспондента = Неопределено Тогда
				...
				Для Каждого СтрокаНастройкиКорреспондента Из НастройкиКорреспондента Цикл
					Если СтрокаНастройкиКорреспондента.Версия <> ВерсияФормата Тогда
						Продолжить;
					КонецЕсли;
...
Процедура ЗаполнитьСтруктуруНастроекXDTOКорреспондента(СтруктураНастроек, Header, ФорматСодержитВерсию = Истина) Экспорт
	...
	Если Не Header.Установлено("AvailableObjectTypes") Тогда
		Возврат;
	КонецЕсли;
...
Процедура ИнициализироватьТаблицуПоддерживаемыхКорреспондентомОбъектовФормата(ТаблицаОбъекты, Режим)
